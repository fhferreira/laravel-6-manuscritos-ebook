...
```
Para iterar as postagens vindas do controller, neste caso o `@forelse` fará o seguinte: Se não houverem postagens na base ele cairá na execução do bloco do `@empty`, onde adicionamos um tr com td e a mensagem `Nada Encontrado!`. Veja o bloco do `@empty` abaixo:
...
@endforelse
```
Existindo posts, que é o nosso caso, teremos a exibição da tabela com as postagens. Vamos analisar o bloco do `@forelse`. Veja somente ele abaixo:
...
O que vale destacar aqui é a utilização da diretiva de controle condicional para exibição da mensagem `Ativo` ou `Inativo` dentro de um badge provido pelo Bootstrap.css, usei também no campo da data de criação da postagem a função date do PHP para formatação de data do próprio PHP. Temos ainda para a coluna de **Ações** o botão de edição onde linkamos a rota de edição usando o apelido dela por meio da função helper route e passando o id dinâmico do post como segundo parâmetro.
...
Adicione o trecho abaixo, logo após o fechamos da tag table da sua view `index.blade.php`:
